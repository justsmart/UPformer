...
class UGTRNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1,T=16,K=50, classes=2, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=255), BatchNorm=nn.BatchNorm2d, pretrained=True, dataset_name='', args=None):
        ...
        self.transformer = build_transformer(self.hidden_dim, dropout, nheads=8, dim_feedforward=2048, enc_layers=3, dec_layers=3, pre_norm=True,decoder_input_len=45*45)
    ...
    def forward(self, x, y=None):
        ...
        # transformer
        # x, self.m_items,gathering_loss, spreading_loss = self.transformer(x, residual, position_encoding,self.m_items,self.training) #self.query_embed.weight, position_encoding, residual)

        '''
        x, t_loss,vis_att = self.transformer(x, residual, position_encoding)
        vis_att = vis_att.transpose(1,2)
        vis_att_size=vis_att.shape
        vis_att=vis_att.view(vis_att_size[0],vis_att_size[1],int(vis_att_size[2]**0.5),int(vis_att_size[2]**0.5))
        '''
        #################
        x=residual
        vis_att=None
