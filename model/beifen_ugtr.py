...
class UGTRNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True,
                 criterion=nn.CrossEntropyLoss(ignore_index=255), T=16, K=80, BatchNorm=nn.BatchNorm2d, pretrained=True,
        ...
        self.transformer = build_transformer(self.hidden_dim, dropout, nheads=8, dim_feedforward=2048, enc_layers=3,
                                             dec_layers=3, pre_norm=True, decoder_input_len=45 * 45)
    ...
    def forward(self, x, y=None):
        ...
        # transformer
        x, self.m_items, gathering_loss, spreading_loss = self.transformer(x, residual, position_encoding, self.m_items,
                                                                           self.training)  # self.query_embed.weight, position_encoding, residual)
        # if x.size(1) == 1:
        ...
        if self.training:
            ...
            return torch.sigmoid(x), ((std3 - std3.min()) / (std3.max() - std3.min())), mean3  # , uncertainty
